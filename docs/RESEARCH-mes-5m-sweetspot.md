...
> to maximize statistical robustness. The operator deliberately **inverted** that rule. The goal
> now is to surface the **rare, high-conviction A+ setups** — the seldom-firing confluence trades
> a discretionary trader actually waits for — not the 200-trade "buy every dip" configs. We pick
> the featured "sweet spot" as the **best Ret/DD among the configs with fewer than 30 trades.**
>
> **This trades statistical robustness for selectivity, on purpose, and the cost is real:** fewer
> than 30 trades means **wide confidence intervals and high fluke risk.** Every crowned config
> below is a **small sample** — a *hypothesis*, never a statistically proven edge. We show each
> config's trade count next to its metrics so the thinness is always visible, and (see the
> walk-forward) we do not pretend a thin in-sample winner is tradeable.
...
> numbers.** Note that under the selectivity cap the crowned config has only ~26 trades, so the
> exact decimals are *more* sensitive to new bars than a high-count config would be — read the
> ranking as suggestive, not stable to the decimal.

**The sweet spot (under the cap):** the **green triangle + a recent TD7 exhaustion, in the
cash session, exited on a 1.5-ATR stop with a 3:1 reward target** (`fractal+TD7+RTH ·
ATR1.5×RR3`) — about **+2.4% return, Ret/DD ≈3.3, Sharpe ≈2.9, 50% win rate, but only ~26
trades.** On a risk-adjusted basis it beats **buy-and-hold MES** (≈+6.5% return but Ret/DD only
≈0.7 — buy-and-hold makes more raw return while eating a far bigger drawdown). (Ret/DD counts
*in-trade* drawdown — the worst the equity dipped mid-trade — so it is a conservative, honest
risk figure.) **26 trades is a thin sample: treat the ratio as suggestive, not proven.**
...
sample is far too small to read the ratio at all.)

**Walk-forward (the honest read — and it is sobering).** We tuned the selective (under-cap)
risk-adjusted-best config on the first 60% — using the **same cap the headline sweet spot uses**
— warming the rolling indicators from pre-split history as a real forward test would, then ran
that frozen config on the unseen last 40%. Because the in-sample winner is already rare (under
30 trades), its held-out 40% slice contains only a **handful of trades (n≈3)** — and **out of
sample it went slightly negative (≈−0.2% return, Ret/DD ≈−0.2).** In plain terms: **the thin
in-sample winner did not survive validation.** This is the predictable cost of the selectivity
cap, stated honestly — a 26-trade in-sample edge splits into a ~3-trade out-of-sample test,
which is far too small to confirm anything. The cap surfaces the rare setups the operator wants
to see, but it cannot, on five months of data, prove any of them are real.

### Bottom line

> **Under the operator's selectivity cap (only setups firing <30 times in 5 months are
> eligible), the featured sweet spot is the green triangle + DeMark TD7 exhaustion + cash-session
> filter on a 1.5-ATR / 3:1 stop-target — Ret/DD ≈3.3, Sharpe ≈2.9 in-sample, but on just ~26
> trades. That is a RARE, high-conviction pattern, exactly what the cap is meant to find — and
> exactly why it is fragile: walk-forward out-of-sample, the thin in-sample winner went slightly
> NEGATIVE on a ~3-trade held-out slice. The cap deliberately trades statistical robustness for
> selectivity, so this is a _hypothesis to forward-test_ over many more months, NOT a proven or
> deployable edge. With this few trades, neither the in-sample ratio nor the OOS miss is
> statistically decisive — the honest verdict is "promising rare setup, unproven on this
> sample."**
...
documented, reproducible tie-break). Each is ranked the same way (Ret/DD under the same <30-trade
selectivity cap) and walk-forward validated.
...
What we found (exact figures in the HTML render; the stable picture — and remember, **the cap
makes every one of these a small sample**):

- **Short-only** under the cap is crowned on a config that fires only **~2 trades** — essentially
  no sample at all. It looks mildly positive in-sample on those couple of trades, then goes
  **negative OOS** (≈−0.2% return, **Ret/DD ≈ −0.6, on ~2 OOS trades**). There is nothing to
  conclude from 2 trades except that it is uninformative.
- **Combined long+short** under the cap (~12 trades) also goes **slightly negative out-of-sample**
  (combined OOS ≈−0.1%, Ret/DD ≈−0.2 on ~4 trades) — it does **not** beat the (also-thin) long
  side OOS. The shorts add nothing once the regime is accounted for.
- **The TD-timed LONG is still the best in-sample config**, but under the cap *it too* fails its
  ~3-trade OOS test (see the walk-forward above). The selectivity cap leaves no direction with a
  large-enough OOS sample to declare a survivor — all three are thin.
...
Under the selectivity cap the short sample shrinks to a literal handful, so the point is doubly
true: there is neither a fair regime nor a usable sample size here. The short signal is
mechanically the exact reflection of the long one, so if the long signal is real, the short
signal is a **credible candidate the moment the regime cooperates** — but this window cannot
prove it either way.

### Bottom line (long/short)

> **Favor the LONG, but hold all conclusions loosely.** On this 5-month UP regime the derived
> short mirror **subtracts**: short-only and combined both go negative out-of-sample. That is
> exactly what theory predicts when you evaluate shorts in a sustained uptrend — **not** evidence
> the short signal is bad, only that you shouldn't short a rising market. The selectivity cap
> compounds the uncertainty: it crowns rare configs (long ~26, combined ~12, short ~2 trades), and
> at those counts the out-of-sample slices are tiny, so **no direction has a statistically usable
> OOS sample.** A fair short evaluation **needs a down/chop regime**, and any of these configs
> needs **many more months** before its ratio means anything. Treat the whole study as a set of
> **hypotheses to forward-test**, not live edges.
...
1. **Forward-test on paper** (the only honest validation on a thin, sub-30-trade sample): run the
   featured sweet spot (triangle + TD7 exhaustion + RTH, 1.5-ATR / 3:1 exit) live-but-simulated
   for several quarters and compare to this study's stats. With only ~26 in-sample trades, a
   forward test that accumulates more trades is the *only* way to tell whether the cap surfaced a
   real rare edge or a small-sample fluke. A real regime change is the further true test.
...
also optimistic on a short window. Selectivity **cap** = `MAX_TRADES=30`; the **featured sweet
spot** (`pick_sweet_spot`) is the best Ret/DD among configs with `n < MAX_TRADES` (so only the
rare, low-count setups are eligible — the operator's deliberate inversion of the old robustness
floor; falls back to all rows only if nothing is under the cap). The short/combined picks use
`capped_best`, which returns `None` unless at least one config is under the cap (a hard gate, so
a busy config is never published as a thin-rare "best"). Walk-forward: 60/40 split, tune Ret/DD
in-sample **under the same cap** (indicators warmed from `WARMUP_BARS=240` pre-split bars), run
the frozen config OOS, and also report the best-with-hindsight OOS config (if the frozen one is
far worse than hindsight, it was fit). Because the in-sample winner is rare by construction, its
OOS slice is tiny (often <5 trades); the OOS-survival gates are set low (`n ≥ 3`) accordingly,
and the report flags every OOS read as a small sample. `walk_forward(df, mode=)` runs the same
validation for **each** of long / short / combined.

### Cost / leverage note
MES = $5/point. The sweet spot's ≈+2.4% ≈ the equity-curve return of a single-instrument
